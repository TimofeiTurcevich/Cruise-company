...
public class FavoriteServlet extends HttpServlet {
    static final Logger logger = LogManager.getLogger(FavoriteServlet.class);
    ...
    public void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        try{
            logger.info("Favorite#DoGet");
            User tempt = (User) req.getSession().getAttribute("user");
            List<Voyage> voyages = ticketsDAO.getFavoriteVoyages(tempt.getId(),shipDAO,stationDAO,path);
            if(req.getParameter("page")==null){
                req.getSession().setAttribute("favoriteTickets",voyages);
                logger.info("Favorite tickets get");
                req.getRequestDispatcher("favorites.jsp?page=1").forward(req,resp);
                ...
                logger.info("Favorite tickets page = " + req.getParameter("page"));
